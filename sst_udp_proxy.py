...
def udp_proxy():
	try:
		with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as listen_socket:
			listen_socket.bind(('0.0.0.0', LOCAL_PORT))
			...
			with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as forward_socket:
				while True:
					try:
						data, client_address = listen_socket.recvfrom(4096)
						forward_socket.sendto(data, (DESTINATION_HOST, DESTINATION_PORT))
						print(f"Forwarded {len(data)} bytes from {client_address} to {DESTINATION_HOST}:{DESTINATION_PORT}")
						print(data)
